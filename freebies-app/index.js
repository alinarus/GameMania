...
app.get('/favorites', authenticateToken, (req, response) => {
  ...
  request.get('http://freebies-favorites:4102/favorites', options, (err, res, body) => {
    if (err) { return console.log(err); }
    response.json(body);
  });
});
...
app.delete('/favorites/:id', authenticateToken, (req, response) => {
  ...
  request.delete(`http://freebies-favorites:4102/favorites/${id}`, options, (err, res, body) => {
    if (err) { return console.log(err); }
    response.json(body);
  });
});
...
app.post('/favorites', authenticateToken, (req, response) => {
  ...
  request.post('http://freebies-favorites:4102/favorites', options, (err, res, body) => {
    if (err) { return console.log(err); }
    response.json(body);
  });
});
